#!/bin/sh
# Genera el PDF de las diapositivas a partir de diapositivas.html.
...
OUT="${OUT:-$(cd "$HERE/.." && pwd)}"
NAME="${NAME:-Diapositivas - Punto 1 Introduccion}"
...
awk 'BEGIN{ while((getline l < "'"$HERE"'/fonts.css")>0) f = f l "\n"
     ...
     {print}' "$HERE/diapositivas.html" > "$HERE/build.html"
